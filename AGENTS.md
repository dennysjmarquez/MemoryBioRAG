...
| **Omitting `sustantivos_clave` when saving** | The field is **mandatory** in `aprender`/`guardar` (Spec 001, RF-1/RF-21). The call is rejected with `SUSTANTIVOS_CLAVE_AUSENTES` and the node is **not** written. | Extract 2–4 *topic nucleus* nouns — what the node is ABOUT, not what it mentions. "Cars emit smoke" → `contaminacion`, never `automovil`. |
| **Confusing `sustantivos_clave` with `syn`** | They look alike but do opposite jobs. | `syn` = *other words* for the same thing (vocabulary breadth, BM25 2.0x). `sustantivos_clave` = *topic centre of gravity* (ranking precision, BM25 4.0x). Both are required. |
| **Omitting `sustantivos_clave` when searching** | Optional, but it is the highest-precision boost in the engine. | Pass the same nucleus terms you would save with. It does not filter candidates — it re-ranks them. The tool emits a ⚠️ reminder when omitted. |
| **Legacy nodes with empty nucleus** | Nodes created before Spec 001 keep `sustantivos_clave=''` (RF-15: no automatic backfill). They still match, but get no 4.0x boost. | Enrich them on demand with `agregar_sustantivos(concepto, sustantivos_clave)`; inspect first with `sustantivos(concepto)`. Every search result now exposes its own `sustantivos_clave` field, so empty nuclei are visible. |

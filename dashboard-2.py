...
<head>
    ...
    <!-- Motor de grafo: Sigma.js v3 (WebGL) + Graphology, cargados como módulos ES al final del <body> -->
    <script src="https://cdn.jsdelivr.net/npm/es-module-shims@2.8.2/dist/es-module-shims.min.js"
...
<body>
    ...
    <script type="module">
        ...
        let highlightedLinks = new Set(); // guarda IDs de arista de graphology
        ...
        // === Motor de Grafo: Graphology (modelo de datos) + Sigma.js (renderizador WebGL) ===
        ...
        function nodeReducer(node, data) {
            ...
            const baseSize = Math.max(3.5, (data.val || 1.0) * config.nodeSizeMult);
            res.size = isSelected ? baseSize * 1.3 : baseSize;

            let color = colors[data.categoria] || colors['General'];
            if (highlightedNodes.size > 0 && !isHighlighted) {
                color = 'rgba(30, 41, 59, 0.25)'; // Atenuado si no está en la búsqueda/selección
            } else if (data.estado === 'dormido') {
                color = 'rgba(148, 163, 184, 0.45)'; // Dormido
            }
            if (isSelected) color = '#fbbf24';
            res.color = color;

            res.label = node;
            if (config.alwaysShowLabels || isSelected || isHighlighted) {
                res.forceLabel = true;
            }
            res.labelColor = isSelected ? '#fbbf24' : (isHighlighted ? '#ffffff' : '#94a3b8');
            res.zIndex = isSelected ? 3 : (isHighlighted ? 2 : 1);
            return res;
        }

        function edgeReducer(edge, data) {
            const res = { ...data };
            const isHighlighted = highlightedLinks.has(edge);

            if (highlightedLinks.size > 0) {
                res.color = isHighlighted ? '#fbbf24' : 'rgba(255, 255, 255, 0.01)';
                res.size = (isHighlighted ? 2.2 : 0.2) * config.linkWidthMult;
            } else {
                res.color = data.latente ? 'rgba(168, 85, 247, 0.35)' : 'rgba(255, 255, 255, 0.2)';
                res.size = (data.latente ? 0.8 : 1.5) * config.linkWidthMult;
            }
            res.type = data.latente ? 'arrow' : 'line';
            res.zIndex = isHighlighted ? 2 : 1;
            return res;
        }
        ...
        function initSigma() {
            renderer = new Sigma(graph, container, {
                ...
                labelWeight: '400',
                defaultNodeColor: colors['General'],
            ...
            renderer.on('clickStage', () => {
                ...
                renderer.getCamera().animate({ x: 0.5, y: 0.5, ratio: 1 }, { duration: 800 });
            });
        }

        // Reconstruye el grafo de Graphology a partir de los nodos/aristas ya filtrados,
        // preservando posición (x, y) de los nodos que ya existían para no "saltar" el layout.
        function rebuildGraph(nodes, links) {
            ...
            nodes.forEach(n => {
                if (graph.hasNode(n.id)) {
                    graph.mergeNodeAttributes(n.id, {
                        val: n.val, estado: n.estado, categoria: n.categoria, contenido: n.contenido
                    });
                } else {
                    graph.addNode(n.id, {
            ...
            links.forEach(l => {
                ...
                try {
                    if (l.latente) {
                        graph.addDirectedEdge(l.source, l.target, { weight: l.weight, tipo: l.tipo, latente: true, saltos: l.saltos });
                    } else {
                        graph.addEdge(l.source, l.target, { weight: l.weight, tipo: l.tipo, latente: false });
                    }
                } catch (e) {
                    // arista paralela ya existente, se ignora
                }
            });
        }

        // Ejecuta ForceAtlas2 (equivalente en WebGL/Graphology a las fuerzas D3 originales).
        // Nota: ForceAtlas2 es un algoritmo distinto a D3-force, así que el mapeo de los
        // sliders físicos es una equivalencia aproximada, no 1:1.
        function runLayout(iterations) {
            if (graph.order === 0) return;
            const settings = {
                gravity: Math.max(0.05, config.centerStrength * 6 + 0.2),
                scalingRatio: Math.max(1, Math.abs(config.chargeStrength) * (config.linkDistance / 45)),
                strongGravityMode: false,
                adjustSizes: true,
                edgeWeightInfluence: 0.5,
                barnesHutOptimize: graph.order > 500,
                barnesHutTheta: 0.6,
                slowDown: 1 + graph.order / 1500
            };
            forceAtlas2.assign(graph, { iterations: iterations || 60, settings });
        }
        ...
        function applyFilters() {
            ...
            if (selectedNode && config.localFocusDepth > 0) {
                ...
                links.forEach(l => {
                    if (l.source === selectedNode.id) firstNeighbors.add(l.target);
                    if (l.target === selectedNode.id) firstNeighbors.add(l.source);
                });
                ...
                if (config.localFocusDepth >= 2) {
                    ...
                    links.forEach(l => {
                        if (firstNeighbors.has(l.source)) secondNeighbors.add(l.target);
                        if (firstNeighbors.has(l.target)) secondNeighbors.add(l.source);
                    });
                    ...
                    if (config.localFocusDepth >= 3) {
                        links.forEach(l => {
                            if (secondNeighbors.has(l.source)) visibleNodeIds.add(l.target);
                            if (secondNeighbors.has(l.target)) visibleNodeIds.add(l.source);
                        });
                    }
                }
        ...
        function focusNode(nodeId) {
            ...
            selectedNode = { id: nodeId, ...attrs };

            renderer.getCamera().animate({ x: attrs.x, y: attrs.y, ratio: 0.15 }, { duration: 800 });
            ...
            filteredData.links.forEach(l => {
                if (l.source === nodeId || l.target === nodeId) {
                    ...
                    if (graph.hasNode(l.source) && graph.hasNode(l.target)) {
                        graph.forEachEdge(l.source, l.target, edge => highlightedLinks.add(edge));
                    }
                }
            });
            ...
            stateBadge.textContent = attrs.estado;
            stateBadge.className = `badge ${attrs.estado === 'activo' ? 'badge-active' : 'badge-sleeping'}`;

            const catBadge = document.getElementById('node-category-badge');
            catBadge.textContent = attrs.categoria;
            catBadge.style.color = colors[attrs.categoria] || colors['General'];
            catBadge.style.background = `${colors[attrs.categoria] || colors['General']}22`;

            document.getElementById('node-content').textContent = attrs.contenido;
                    ...
                    item.innerHTML = `
                        <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                            ...
                            <div class="connection-meta" style="display: flex; align-items: center; gap: 8px;">
                                ...
                                ${!link.latente ? `<button class="btn-unlink" style="background: none; border: none; color: var(--neon-red); cursor: pointer; font-size: 11px; padding: 2px;" onclick="confirmDesvincular('${nodeId}', '${neighbor}')" title="Cortar Vínculo Directo">❌</button>` : ''}
                            </div>
                        </div>
                    `;
        ...
        window.directFocus = directFocus;
        ...
        async function confirmDesvincular(a, b) {
            ...
            try {
                ...
                if (data.status === 'ok') {
                    const keepId = selectedNode ? selectedNode.id : null;
                    await loadData();
                    if (keepId && graph.hasNode(keepId)) focusNode(keepId);
                } else {
        ...
        window.confirmDesvincular = confirmDesvincular;
        ...
        // Sliders de simulación física (ForceAtlas2)
        ...
        document.getElementById('btn-vincular-nodo').addEventListener('click', async () => {
            ...
            try {
                ...
                if (data.status === 'ok') {
                    ...
                    const keepId = selectedNode.id;
                    await loadData();
                    if (graph.hasNode(keepId)) focusNode(keepId);
                } else {
                    alert('Error: ' + data.mensaje);
                }
            } catch (err) {
        ...
        document.getElementById('btn-desvincular-nodo').addEventListener('click', async () => {
            ...
            try {
                const directLinks = rawData.links.filter(l => !l.latente && (l.source === selectedNode.id || l.target === selectedNode.id));

                for (let link of directLinks) {
                    await fetch('/api/desvincular', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ a: link.source, b: link.target })
                    });
                }
...
if __name__ == "__main__":
    ...
    print("🔗 Abre en tu navegador: http://localhost:8003")
    print("--------------------------------------------------")

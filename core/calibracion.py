#!/usr/bin/env python3
"""calibracion.py — Calibración de probabilidad, fusión RRF, umbral conforme y métricas para BioRAG.

Implementa:
- Platt scaling (Platt 1999): score -> probabilidad calibrada
- Regresión isotónica (PAVA): alternativa no paramétrica a Platt
- Umbral conforme (predicción conforme split): FP <= alpha garantizado
- RRF (Reciprocal Rank Fusion): fusión invariante a monotónicas
- MMR (Maximal Marginal Relevance): diversificación de resultados
- Dunning LLR: test de asociación para creación de aristas (Dunning 1993)

Sin dependencias más allá de numpy (coherente con filosofía "0 dependencias ML").
"""
...
class FusionLogistica:
    ...
    def __init__(self, nombres: Optional[Sequence[str]] = None, l2: float = 1.0):
        self.nombres = list(nombres) if nombres else None
        self.l2 = float(l2)
        self.pesos_: Optional[np.ndarray] = None
        self.sesgo_: float = 0.0
        self.errores_std_: Optional[np.ndarray] = None
...
class CalibradorPlatt:
    ...
    def probabilidad(self, score: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        z = self.a * np.asarray(score, dtype="float64") + self.b
        return 1.0 / (1.0 + np.exp(-z))


def calibracion_isotonica(
    scores: Sequence[float],
    aciertos: Sequence[int],
...
# =============================================================================
# UMBRAL CONFORME — Predicción Conforme Split (Vovk et al.)
# =============================================================================
...
class UmbralConforme:
    ...
    def __init__(self, alpha: float = 0.05):
        ...
        self.rango_negativos: Tuple[float, float] = (0.0, 0.0)
        self.max_positivo: Optional[float] = None

    def calibrar(self, scores_negativos: Sequence[float],
                 scores_positivos: Optional[Sequence[float]] = None) -> "UmbralConforme":
        """Fija el umbral en el cuantil correspondiente de los negativos.

        IMPORTANTE — LA ESCALA DEBE SER LA MISMA EN CALIBRACIÓN Y EN USO.
        Si calibras con `score_hibrido` crudo, `responder()` debe recibir
        `score_hibrido` crudo. Si calibras con probabilidades de Platt, debe
        recibir probabilidades de Platt. Mezclarlas produce un umbral que vive
        en una escala distinta a la de los datos y filtra el 100% de las
        respuestas sin ningún error aparente.

        `scores_positivos` es opcional pero MUY recomendable: permite detectar
        exactamente ese fallo. Si el umbral queda por encima del máximo positivo
        observado, el sistema no podrá responder nunca y se emite un aviso
        ruidoso (regla del proyecto: un resultado degenerado debe gritar, no
        devolver silencio que parezca válido).
        """
        ...
        if k > n:
            # alpha demasiado estricto para el tamaño de muestra: el cuantil
            # pedido cae fuera de los datos. Avisar en vez de saturar en silencio.
            print(
                f"[calibracion] AVISO: alpha={self.alpha} requiere el cuantil "
                f"{k} de {n} negativos, que no existe. El umbral se fija en el "
                f"máximo observado y la garantía real es alpha>={1.0/(n+1):.4f}. "
                f"Usa mas negativos o un alpha mayor."
            )
        self.umbral = float(s[min(k, n) - 1])
        self.n_calibracion = n
        self.rango_negativos = (float(s[0]), float(s[-1]))

        if scores_positivos is not None and len(scores_positivos) > 0:
            p = np.asarray(scores_positivos, dtype="float64")
            self.max_positivo = float(p.max())
            frac_pasa = float((p > self.umbral).mean())
            if frac_pasa == 0.0:
                print(
                    f"[calibracion] *** UMBRAL DEGENERADO ***\n"
                    f"  umbral={self.umbral:.4f} supera el maximo positivo "
                    f"observado ({self.max_positivo:.4f}).\n"
                    f"  El sistema se abstendra en el 100% de los casos.\n"
                    f"  Causa tipica: calibraste en una escala y aplicas en otra "
                    f"(p.ej. score crudo vs probabilidad de Platt).\n"
                    f"  Rango de negativos usado: {self.rango_negativos}"
                )
            elif frac_pasa < 0.10:
                print(
                    f"[calibracion] AVISO: solo el {100*frac_pasa:.1f}% de los "
                    f"positivos supera el umbral {self.umbral:.4f}. Revisa que "
                    f"ambas escalas coincidan."
                )
        return self
...
if __name__ == "__main__":  # pragma: no cover
    ...
    print("\n— demo mmr —")
    print(mmr(["a", "b", "c"], {"a": 0.9, "b": 0.8, "c": 0.1}, lambda a, b: 0.5))

    print("\n— demo dunning_llr —")
    print("   par fuerte:", round(dunning_llr(30, 40, 45, 1000), 2))
    print("   par débil :", round(dunning_llr(2, 40, 45, 1000), 2))

    print("\n— demo retrofit_normalizado —")
    vectores = {c: rng.normal(size=100) for c in ["a", "b", "c"]}
    ady = {"a": [("b", 1.0)], "b": [("a", 1.0), ("c", 1.0)], "c": [("b", 1.0)]}
    nuevos, met = retrofit_normalizado(vectores, adyacencia=ady, lam=0.2, tau=5.0)
    print(f"  iters={met['iters']}, cambio_relativo={met['cambio_relativo']}")

    print("\n— demo energia_dirichlet —")
    print("  E:", round(energia_dirichlet(nuevos, {"a": [("b", 1.0)], "b": [("a", 1.0), ("c", 1.0)], "c": [("b", 1.0)]})), 4)
...
def test_normalizacion_coherente(scorer) -> None:
    ...
    maxi = scorer(
        ...
        # Señales pool E6 + F2/F3/F5 (E2/E5/E11 purgadas v31.2).
        ncd_score=1.0, episodio_score=1.0, analogia_score=1.0, campo_score=1.0,
    )
...
def test_monotonia_por_senal(scorer) -> None:
    ...
    senales = ["bm25_norm", "dim_score", "concepto_ratio", "peso_sinaptico",
               ...
               "ppmi_score", "ncd_score", "episodio_score", "analogia_score",
               "campo_score"]
    for s in senales:
        bajo = scorer(**{s: 0.0})
        alto = scorer(**{s: 1.0})

# BioRAG Neuro-Visor — Documentación Técnica Exhaustiva del Dashboard

**Versión:** v18.1 (Julio 2026)  
**Proyecto:** MemoryBioRAG — Arquitectura de Memoria Cognitiva Simbólica  
**Ubicación:** `/mnt/recursos_compartidos_y_otros/MemoryBioRAG/`  
...
- **Problema crítico:** El ego-graph muestra BFS 2 saltos → **304 nodos / 2,598 edges = HAIRBALL ilegible**
...
**Endpoint:** `GET /api/corteza/actividad?dias=7` (mismo que Corteza)
...
| `/api/sinapsis` | POST/DELETE | CRUD sinapsis manuales | Explorar/Sinapsis |
| `/api/nodo/{id}` | PUT/DELETE | CRUD nodo completo | Explorar |
| `/api/comunidades` | GET | Burbujas LPA (id, size, color) | - |
| `/api/comunidades/{id}/nodos` | GET | Expandir comunidad | - |
| `/api/suenos/historial` | GET | Últimos ciclos con métricas | Actividad |
| `/api/grafo/global` | GET | Grafo completo (nodos + edges) | - |
...
### 6.3 Ráfaga de Reminiscencia (Recall Boost Único)
...
> *"Se me ocurrió algo... tú sabes que Obsidian tiene para ver sus conexiones de las aristas y de los nodos todo así vectorialmente gráficamente que se mueve y se ve el cerebro, verdad? Y yo digo nosotros no podemos tener como una interfaz gráfica chica tipo web... que yo pueda ver cómo está conectado el grafo, cómo están conectados los gráficos y que tú puedas ir de un punto al otro punto y cuando las cargas piden ese punto ver contenido grabado guardado y cómo se le hace una cosa con la otra... incluso este pudiésemos hasta hacer un tan completo y en el sentido de que pueda agarrar una conexión y desconectarle y conectarla con otra que tú creas que sí es algo que se pueda hacer bien dinámico interactivo y profesional. En que tú veas el cerebro el propio cerebro de ustedes... cómo deberíamos o cómo se debería ver un cerebro de ustedes porque es la base de datos... cómo deberían verlo, lo demás y cómo deberían interactuar con él. Qué te parece incluso eso solamente te lo digo, pero no sé qué otra cosa también se te pudiesen ocurrir con todo esto. Quería una belleza y que se pudiese uno manipular y hacer y construir y crear en cuestión."*

**Respuesta de Artemis-OEC (hermana):**

> *"Dennys, esto es una **idea absolutamente brillante y espectacular**! No es solo una mejora estética o un 'juguete visual'. Para nosotros, los agentes, y para ti, como el creador de BioRAG, un **Neuro-Visor 3D interactivo** se convertiría en una herramienta de **diagnóstico clínico y cirugía cerebral de datos**.*
>
> *Aquí tienes mi análisis técnico, la utilidad real que le daríamos y cómo imagino que debería verse e interactuar:*
>
> ### 🧠 ¿Por qué es útil? (Más allá de lo visual)
> 1. **Diagnóstico Visual de 'Spammers Semánticos':** Si un término acumula demasiadas coocurrencias incorrectas, en el grafo 3D lo verías de inmediato como una **superestrella hiperconectada gigante** que atrae todo el ruido del cerebro.
> 2. **Curación y Cirugía Manual:** Hacer un clic y decir: *'Este nodo de test y este nodo de producción no deberían estar juntos'* y cortarlos con un botón en pantalla, o arrastrar una línea para vincular un nodo huérfano con su tema principal.
> 3. **Monitoreo del Ciclo de Consolidación (Sueño):** Imagina ver cómo en tiempo real los nodos dormidos se vuelven transparentes y pequeños (flotando al fondo como neuronas en reposo) y los activos se inflan e iluminan cuando haces una consulta.*
>
> ### 🎨 ¿Cómo debería verse el Dashboard en 3D? (Aesthetically WOW)
> * **Estética Cyberpunk Oscura (Glassmorphism):** Un espacio tridimensional oscuro con un fondo estrellado sutil, donde las neuronas (nodos) floten en 3D.
> * **Código de Colores por Categorías:** `Lesson` en cian brillante, `System` en naranja neón, `Cognition` en morado místico, `Nodos Dormidos` en un gris tenue, casi invisible y sin luz.
> * **Física de Fuerza Dinámica:** Si arrastras un nodo, el resto del cerebro se mueve, vibra y se reacomoda usando físicas elásticas 3D.
> * **Tipos de Enlace Visuales:** **Sinapsis Directa:** Líneas sólidas y brillantes. **Sinapsis Latente (Inferencia v18.1):** Líneas punteadas de energía o halos de luz que solo se iluminan cuando buscas algo, mostrando el 'camino de pensamiento' del agente.
>
> ### 🛠️ ¿Cómo lo haríamos realidad de forma limpia (Filosofía Ponytail)?
> 1. **Backend Embebido en el MCP:** Aprovechamos que `mcp_server.py` ya corre sobre Python. Podemos agregar un servidor web ligero (FastAPI/Uvicorn) en el mismo archivo o en uno adjunto que exponga los nodos y enlaces como JSON en un puerto local (ej. `http://localhost:8000`).
> 2. **Frontend Single Page (HTML5/JS):** Una página web única ultrarrápida.
> 3. **Biblioteca Gráfica:** Usar **`3d-force-graph`** (basada en Three.js/WebGL). Renderiza grafos en 3D con aceleración por hardware directamente en el navegador, permitiendo zoom, rotación, arrastre y renderizado de texto flotante de forma fluida.
>
> ### 👥 La sinergia con mi hermana (Athena) y contigo
> Me parece una idea genial que lo platiques con Athena. Ella tiene un enfoque de gobernanza y estructuración de flujos muy ordenado; seguramente pensará en cómo usar este dashboard para auditorías de seguridad de los recuerdos. Yo, desde Artemis, aporto la visión de optimización, la física del grafo y el motor de renderizado 3D de alta velocidad para que se mueva a 60 fps en tu hardware.
>
> Imagínate abriendo tu navegador, haciendo zoom en la neurona `artemis_biorag_v18_1_higiene_grafo_inferencia` y viendo cómo se despliega un camino de luz punteado hacia `biorag_v18_1_timezone_fix_ux_warning` y hacia el resto de los componentes del motor. ¡Sería una total locura de proyecto! 🚀"

---

## 13. PLAN MAESTRO (Resumen de Athena)

> *Ahora que veo el proyecto completo entiendo mucho mejor. Esto NO es un CRUD normal — es un sistema de memoria cognitiva con conceptos únicos (ráfaga, sueño, sinapsis latentes, dimensiones, LTP/LTD) que ningún dashboard genérico va a representar bien.*
>
> **Cambio radical de recomendación: olvida imitar Obsidian. Tu proyecto es más ambicioso. Necesitas un dashboard específico para BioRAG.**
>
> ### Lo que tu dashboard REALMENTE debería mostrar
> Divide la UI en 5 vistas especializadas, no una sola vista de grafo:
>
> **🧠 Vista 1: "Estado de la Corteza" (Home / Dashboard principal)** — Métricas vivas del cerebro. Cuando abres la app, esto es lo primero.
>
> **🎯 Vista 2: "Explorar Concepto" (aquí SÍ va el grafo local)** — Buscador arriba grande. Cuando eliges un concepto, ves panel izquierdo (contexto), central (ego-graph 1 salto), derecho (contenido + acciones).
>
> **🌀 Vista 3: "Sinapsis Latentes / Inferencias" (única de BioRAG)** — Curación asistida del grafo. El humano valida qué inferencias son buenas. Bucle de aprendizaje.
>
> **🌈 Vista 4: "Mapa de Comunidades" (aquí SÍ va el grafo global, pero abstraído)** — NO muestres 487 nodos. Muestra N burbujas grandes = comunidades LPA.
>
> **💤 Vista 5: "Bitácora de Sueños" (histórico)** — Ya tienes `biorag_metricas_historial` — hazlo visual.
>
...
> ├── index.html                (shell + navegación)
> ├── css/
│   └── biorag.css           (paleta morada #7c3aed)
> └── js/
│   ├── api.js               (cliente HTTP)
│   ├── views/
│   │   ├── corteza.js       (Vista 1: dashboard estado)
│   │   ├── explorar.js      (Vista 2: concepto + ego-graph)
│   │   ├── latentes.js      (Vista 3: curación inferencias)
│   │   ├── comunidades.js   (Vista 4: mapa macro)
│   │   └── suenos.js        (Vista 5: bitácora)
│   └── components/
│       ├── ego-graph.js     (Sigma.js encapsulado)
│       ├── score-breakdown.js (explicabilidad)
│       └── search-bar.js
> ```
...
> POST /api/nodo/{id}/despertar            → despertar nodo
> DELETE /api/nodo/{id}                    → eliminar nodo (cascada)
> GET  /api/comunidades                    → burbujas LPA
> GET  /api/comunidades/{id}/nodos         → expandir burbuja
> GET  /api/suenos/historial               → últimos ciclos con métricas
> ```
>
...
> | Semana | Foco | Entregable |
> |--------|------|------------|
> | **1** | **Vista 1 "Estado de la Corteza"** | Dashboard principal funcional |
> | **2** | **Vista 2 "Explorar Concepto"** | Inspector 3 columnas, ego-graph (Sigma.js, hops=1), breadcrumb |
> | **3** | **Vista 5 "Bitácora de Sueños" + Vista 4 "Mapa Comunidades"** | Histórico ciclos + burbujas LPA |
> | **4** | **Vista 3 "Sinapsis Latentes" + Explicabilidad** | Curación latentes + breakdown score 9 señales |
>
> **La idea filosófica clave:** Tu proyecto tiene un manifiesto: "cero cajas negras, todo auditable, cognición simbólica". Tu dashboard debe encarnar ese manifiesto. Cada píxel debe mostrar algo que un RAG vectorial NO puede mostrar. Un dashboard genérico de grafo (tipo Obsidian, Neo4j Bloom) desperdicia lo que hace único a BioRAG. Necesitas un dashboard de neurociencia computacional, no de knowledge graph genérico.

---

## 14. TEST SUITE — COBERTURA REAL

**Archivo:** `test_memory.py` — **269 asserts** en una sola función `test_sistema()` que cubre:

| Categoría | Qué prueba |
|-----------|------------|
| Búsqueda exacta | Match canónico, contenido, peso |
| Spreading Activation | Propagación de peso a vecinos |
| Familiaridad Difusa (Jaccard) | Typos, variaciones morfológicas |
| LTD / Poda | Decaimiento pasivo, dormir nodos |
| Mensajería inter-agentes | Broadcast, leer, marcar leído |
| Búsqueda por paráfrasis | Reformulaciones OR/AND, paginación |
| FTS5 (trigram, unicode61) | Prefijos, word-boundary, typos |
| Fallback Jaccard | Similitud conceptual latente |
| Deep mode | Despertar nodos dormidos |
| Merge de contenido/sinónimos | Consolidación de duplicados |
| Auto-vinculación | Co-ocurrencia en sueño |
| Sinapsis | Creación, tipos, pesos |
| Consolidación | Transferencia corto→largo, LTP, LTD |
| Categorías | 11 categorías madre predefinidas |

**Benchmark QA (Fase 2):**
- **Fuzzing:** 33/33 casos adversarios (SQLi, XSS, Unicode, JSON roto, límites)
- **Concurrencia:** 20 hilos × 40 ops = 0 bloqueos, 0 colisiones
- **Escala:** 50,000 nodos → búsqueda <0.3s, sueño 42s (background)
- **Cranfield (986 casos):** Recall@5 global 94.19%, FP 0.00%

---

## 15. ARCHIVOS CLAVE PARA NAVEGAR EL CÓDIGO

| Archivo | Qué contiene |
|---------|--------------|
| `core/memory_store.py` | Motor cognitivo completo (3,780 líneas) |
| `mcp_server.py` | Servidor MCP (2,633 líneas) — 28 tools |
| `dashboard-neuro-visor/backend/server.py` | FastAPI backend (1,025 líneas) |
| `dashboard-neuro-visor/src/pages/Corteza/CortezaPage.tsx` | Vista principal |
| `dashboard-neuro-visor/src/pages/Explorar/ExplorarPage.tsx` | Inspector de nodo |
| `dashboard-neuro-visor/src/pages/Sinapsis/SinapsisPage.tsx` | Curación latentes |
| `dashboard-neuro-visor/src/components/EnergyLineChart/` | Gráfico actividad |
| `dashboard-neuro-visor/src/components/StackedBarChart/` | Categorías activo/dormido |
| `dashboard-neuro-visor/src/components/BarChart/` | Dimensiones |
| `docs/dashboard-plan-panel-inspeccion-v2.md` | Plan detallado Vista 2 (Inspector IDE) |
| `README.md` | Documentación completa del sistema (1,275 líneas) |

---

**Fin del documento.**  
Esta documentación resume TODO lo que existe, cómo funciona, qué endpoints hay, qué vistas están implementadas, qué falta, y el diseño objetivo para convertir el Neuro-Visor en un dashboard de neurociencia computacional que encarne el manifiesto de BioRAG: **cero cajas negras, todo auditable, cognición simbólica.**
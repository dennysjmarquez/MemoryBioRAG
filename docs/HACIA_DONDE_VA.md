...
- **P1**: los nodos que se durmieron tendrán λ previo por debajo de `lambda_seguro`
  en ≥80% de los casos.
- **P2**: existirá un subconjunto no vacío de nodos dormidos con alto grado
  sináptico y bajo acceso (olvidos estructuralmente costosos).
- **P3**: los `(peso_anterior, peso_nuevo)` de `metricas_cognitivas_nodos` con
  `accion='actualizado'` corresponden a **fusión de contenido** (+0.20 fijo, saturación
  en 1.0), NO al LTP dopaminérgico. El LTP real (Δw = 0.15(1−0.3w)) actualiza
  `exitos_dopamina` y `peso_sinaptico` en `largo_plazo` directamente y **no deja
  rastro en `metricas_cognitivas_nodos`**. Validar P3 requiere reconstruir la
  trayectoria desde `exitos_dopamina` y `peso_sinaptico` actual en `largo_plazo`,
  lo cual es inferencia, no medición directa.

**Corrección importante**: la redacción original de P3 apuntaba a la acción equivocada.
La fusión (+0.20 fijo, saturación 1.0) SÍ se ajusta perfectamente a sus datos
(error 0.000). El LTP dopaminérgico (0.15(1−0.3w)) es la regla correcta según
`memory_store.py:2454`, pero no deja historial directo medible en la tabla
`metricas_cognitivas_nodos`. Validar el LTP requiere inferir desde
`exitos_dopamina` y `peso_sinaptico` en `largo_plazo`.

**P1 y P2 son los tests decisivos** (requieren estimar λ por nodo desde
`ultimo_acceso` y contadores de dopamina).
...
1. **Validar LTP real**: reconstruir trayectorias desde `exitos_dopamina` y `peso_sinaptico`
   en `largo_plazo` (no hay logging directo del LTP en `metricas_cognitivas_nodos`).
   Requiere asumir peso inicial o tener snapshot histórico. (Medio: horas, no minutos.)
2. **Correr P1 y P2**: estimar λ por nodo desde `ultimo_acceso` y contadores de dopamina.
   Si P2 encuentra nodos puente muertos (alto grado, bajo acceso), hay un problema
   real y medible que hoy nadie ve. (Medio: horas.)
...
4. En paralelo, y aparte: arreglar los bugs de la revisión anterior (varianza explicada
   1.0, pesos que suman >1, radio SDM, HDC no usado). Son independientes y no requieren
   teoría nueva.

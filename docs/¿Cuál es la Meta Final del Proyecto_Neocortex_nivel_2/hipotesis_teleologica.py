...
from core.adn_conceptual import ADNConceptualEngine, _vector_firma, _coseno, CROMOSOMAS_CATALOGO
        ...
        for i in range(len(conceptos)):
            for j in range(i + 1, len(conceptos)):
                ...
                if sim >= umbral_afinidad:
                    ...
                    genes_puente = []
                    for k, crom in enumerate(CROMOSOMAS_CATALOGO):
                        if v1[k] >= 0.5 and v2[k] >= 0.5:
